...
int main()
{
    ...
    vector<pair<int,int> > jtimeArray;
    cout<<"Enter "<<nj<<" job details with Time and Size respectively:\n";
    double t1,t2;
    for(int i=0;i<nj;i++)
    {
          cin>>j[i].bt>>j[i].jsize;
          jtimeArray[i].first = j[i].bt;
          jtimeArray[i].second = j[i].jsize;
    }
    sort(jtimeArray,jtimeArray+nj);
    ...
    for(time=0;time<total;time++)
    {
        ...
        for(int i=0;i<nj;i++)
        {
            if(j[i].finish == 0)
            {
                if(j[i].jsize == jtimeArray[nextJobIndex].second)
                {
                    nextJobIndex++;
                    next = i;
                    break;
